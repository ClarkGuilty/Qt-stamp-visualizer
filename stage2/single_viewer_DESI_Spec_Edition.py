...
class ApplicationWindow(QtWidgets.QMainWindow):
    def __init__(self, clipboard=None):
        ...
        self.specpath = join(self.stampspath, "spec")
        
        self.has_spectra = os.path.isdir(self.specpath)
        ...
        if self.has_spectra:
            self.all_bands.append(SPECTRA_BAND)
        ...
        self.bands = ['vis_only', 'vis_y', 'vis_y_j'] # Write first the main band.
        ...
        self.plot_layout_area_container.setStyleSheet("background-color: black;")
 ...
 #       self.spectra_figure = Figure(
            #figsize=(8, 2),   # long rectangle
            #layout="constrained",
#            facecolor="black"
#        )
#        self.spectra_canvas = FigureCanvas(self.spectra_figure)
#        self.spectra_ax = self.spectra_figure.subplots()

#        self.spectra_canvas.setStyleSheet("background-color: black")
#        self.spectra_ax.set_axis_off()

        # size policy: wide and short
#        self.spectra_canvas.setSizePolicy(
#            QtWidgets.QSizePolicy.Expanding,
#            QtWidgets.QSizePolicy.Expanding
#        )

#        self.spectra_canvas.setMinimumHeight(120)
        ...
        #self.brecenter.clicked.connect(partial(self.recenter,'INFO'))
        #self.brecenter_original_palette = self.brecenter.palette()
        #list_subclassification_buttons.append(self.brecenter)


        self.recenter_options = [
            ("Info ok + EUCIMG ok", "Info ok + EUCIMG ok [a]", "a"),
            ("Info ok + EUCIMG NO", "Info ok + EUCIMG NO [s]", "s"),
            ("Info NO + EUCIMG ok", "Info NO + EUCIMG ok [d]", "d"),
            ("Info NO + EUCIMG NO", "Info NO + EUCIMG NO [f]", "f"),
        ]

        self.original_recenter_button_style = None
        self.dict_rec2button = {"None": None, "Empty": None}

        self.recenter_buttons = {}  # value -> QPushButton

        for value, label, _key in self.recenter_options:
            btn = QtWidgets.QPushButton(label)
            btn.setCheckable(True)
            btn.setFocusPolicy(Qt.NoFocus)
            btn.clicked.connect(partial(self.set_recenter_from_ui, value))

            if self.original_recenter_button_style is None:
                self.original_recenter_button_style = btn.styleSheet()

            list_subclassification_buttons.append(btn)
            self.recenter_buttons[value] = btn
            self.dict_rec2button[value] = btn
        ...
        #if recenter is not None and recenter != 'None' and recenter != 'Empty':
        #   self.bactivatedrecenter.setStyleSheet("background-color : {};color : white;".format(self.buttonclasscolor))
        #    self.bactivatedrecenter = self.dict_rec2button[recenter]
        ...
        # Recenter option shortcuts (only active when keyboardshortcuts checkbox is enabled)
        self.kz_recenter = QShortcut(QKeySequence('a'), self)
        self.kz_recenter.activated.connect(partial(self.keySetRecenter, "Info ok + EUCIMG ok"))

        self.kx_recenter = QShortcut(QKeySequence('s'), self)
        self.kx_recenter.activated.connect(partial(self.keySetRecenter, "Info ok + EUCIMG NO"))

        self.kc_recenter = QShortcut(QKeySequence('d'), self)
        self.kc_recenter.activated.connect(partial(self.keySetRecenter, "Info NO + EUCIMG ok"))

        self.kv_recenter = QShortcut(QKeySequence('f'), self)
        self.kv_recenter.activated.connect(partial(self.keySetRecenter, "Info NO + EUCIMG NO"))
        ...
        main_layout.addWidget(self.plot_layout_area_container, 88)
        #main_layout.addLayout(self.plot_layout_area, 88)
        #main_layout.addWidget(self.spectra_canvas, 1)
        main_layout.addWidget(button_layout_container, 10)
    ...
    def _set_recenter_value(self, value: str | None):
        """
        Persist recenter value for current image to dataframe + csv.
        If value is None, store 'None' (not 'Empty') so the UI treats it as cleared.
        """
        cnt = self.config_dict['counter']
        assert self.df.at[cnt, 'file_name'] == self.listimage[self.config_dict['counter']]

        self.df.at[cnt, 'recenter'] = (value if value is not None else 'None')
    ...
    @Slot()
    def set_recenter_from_ui(self, value: str):
        """
        Toggle behavior:
        - If clicking a different option: activate it.
        - If clicking the currently active option: clear selection.
        """
        current = self.df.at[self.config_dict['counter'], 'recenter']
        if current == value:
            # unclick -> clear
            self._set_recenter_value(None)
        else:
            self._set_recenter_value(value)

        self.update_recenter_button()

    @Slot()
    def keySetRecenter(self, value: str):
        if self.config_dict['keyboardshortcuts'] is True:
            self.set_recenter_from_ui(value)

    @Slot()
    def clear_recenter(self):
        """Optional: clear selection (you can bind this to a key if you want)."""
        self._set_recenter_value(None)
        self.update_recenter_button()

    def plot_spectrum_in_axis(self, ax):
        fig = ax.figure
        fig.patch.set_facecolor("black")  
        ax.set_facecolor("black")          
        ...
        if not self.has_spectra:
            ax.text(
                0.5, 0.5, "No spectra folder found",
        ...
        spec_file = join(self.specpath, self.filename)

        if not os.path.exists(spec_file):
            ax.text(
                0.5, 0.5, "No spectrum available",
                color="white", ha="center", va="center",
                transform=ax.transAxes
            )
            ax.set_axis_off()
        ...
        img = np.asarray(Image.open(spec_file))
...
#
#        if not self.has_spectra:
#            self.spectra_ax.text(
#                0.5, 0.5, "No spectra folder found",
#                color="white", ha="center", va="center",
#                transform=self.spectra_ax.transAxes
#            )
#            self.spectra_ax.set_axis_off()
...
#        self.spectra_ax.cla()
#        self.spectra_ax.imshow(img, aspect="equal", origin="upper")
#        self.spectra_ax.set_aspect("equal", adjustable="box")
#        self.spectra_ax.set_anchor("C")
#        self.spectra_ax.set_axis_off()
#        self.spectra_canvas.draw()
#        self.spectra_canvas.draw_idle()

#        self.spectra_ax.set_xlim(-0.5, img.shape[1] - 0.5)
#        self.spectra_ax.set_ylim(img.shape[0] - 0.5, -0.5)
    ...
    @Slot()
    def recenter_old(self,val):
        t0 = time()
        cnt = self.config_dict['counter']# - 1
        assert self.df.at[cnt,'file_name'] == self.listimage[self.config_dict['counter']] #TODO handling this possibility better.
        self.df.at[cnt,'recenter'] = val #not self.df.at[cnt,'recenter']
        if self.filetype == _FILETYPE_FITS:
            self.df.at[cnt,'ra'] = self.ra
            self.df.at[cnt,'dec'] = self.dec
            self.df.at[cnt,'pixel_size'] = self.image_pixel_size
...
#        self.plot_spectrum()  
    ...
    def plot_band(self, band, row, scale_min=None, scale_max=None):
        ...
        if band == SPECTRA_BAND:
            self.plot_spectrum_in_axis(ax)
        else:
            path = join(self.stampspath, band, self.filename)
            ...
            ax.imshow(image, origin='upper', cmap=self.config_dict['colormap'], vmin=0, vmax=255)

        ax.set_axis_off()
        self.row_canvas[row][band].draw()

    def plot_band_old(self, band, row, scale_min=None, scale_max=None):
        ax = self.axes[row][band]
        ax.cla()
        ...
        if band == SPECTRA_BAND:
            ...
            image = np.asarray(
                Image.open(join(self.stampspath, band, self.filename))
            )
            ax.imshow(
                image,
                origin='upper',
                cmap=self.config_dict['colormap'],
                vmin=0,
                vmax=255
            )
        ax.set_axis_off()
        self.row_canvas[row][band].draw()
    #    for band in self.plot_layout_rows_widgets[row]:
 #       canvas = self.row_canvas[row][band]

    def plot_band_prev(self, band, row, scale_min = None, scale_max = None):
    ...
    def obtain_df(self):
        ...
        df['recenter'] = ['Empty'] * len(self.listimage)#np.full(len(self.listimage),False)
        return df
    ...
    def update_recenter_button(self):
        ...
        # Reset all buttons to default style + unchecked
        for btn in getattr(self, "recenter_buttons", {}).values():
            btn.setStyleSheet(self.original_recenter_button_style)
            btn.setChecked(False)

        self.bactivatedrecenter = None

        # Nothing selected
        if recenter is None or recenter in ('None', 'Empty'):
            return

        # Select matching button if exists
        button = self.dict_rec2button.get(recenter, None)
        if button is None:
            return

        button.setChecked(True)
        button.setStyleSheet(
            "QPushButton {{background-color : {};color : white;}}".format(self.buttonclasscolor)
        )
        self.bactivatedrecenter = button

    def update_recenter_button_old(self):
        recenter = self.df.at[self.config_dict['counter'],'recenter']
        #if recenter:
        if self.bactivatedrecenter is not None: 
            self.bactivatedrecenter.setStyleSheet(self.original_button_style) 
        ...
        if recenter is not None and recenter != 'None' and recenter != 'Empty':   
            button = self.dict_rec2button[recenter]
            if button is not None:
                button.setStyleSheet("QPushButton {{background-color : {};color : white;}}".format(self.buttonclasscolor))
        #else:
            # print('#########')
